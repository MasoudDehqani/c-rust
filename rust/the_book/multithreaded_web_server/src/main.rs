...
fn main() {
    ...
    listener
        .incoming()
        .for_each(|s| handle_connection_with_buffer(s.unwrap()));
}
...
between two places
*/
fn handle_connection_with_buffer(mut stream: TcpStream) {
...
/*
  this code makes the server wait for an EOF. here tcp stream in the keep-alive connection does not
  have an EOF
  - EOF -> end of file
  ...
  - here we need a buffer reader which reads up to the end of each buffers come the server
  - a buffer is just a chunk of memory where you temporarily store data while moving moving it
  between two places
*/
